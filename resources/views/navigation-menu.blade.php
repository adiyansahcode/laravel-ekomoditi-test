    ...
    <div class="flex justify-between h-16">
      <div class="flex">
        ...
        @if (Auth::user())
          ...
          @can('kriteriaBuahAccess')
            <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
              <x-jet-nav-link href="{{ route('KriteriaBuah.index') }}" :active="request()->routeIs('KriteriaBuah.index')">
                Kriteria Buah
              </x-jet-nav-link>
            </div>
          @endcan
          ...
          @canany(['laporanTransaksiAccess', 'laporanTransaksiDivisiAccess'])
          <!-- Users Management -->
          <div class="hidden space-x-8 sm:-my-px sm:ml-6 sm:flex">
            <x-jet-dropdown align="left" width="60">
              <x-slot name="trigger">
                @php
                $navClass = (request()->routeIs('laporanTransaksi.index') || request()->routeIs('laporanTransaksiDivisi.index'))
                ? 'inline-flex items-center px-1 pt-6 pb-5 border-b-2 border-jets-400 text-sm font-medium leading-5 text-gray-900 focus:outline-none focus:border-jets-700 transition'
                : 'inline-flex items-center px-1 pt-6 pb-5 border-b-2 border-transparent text-sm font-medium leading-5 text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:outline-none focus:text-gray-700 focus:border-gray-300 transition';
                @endphp
                <span class="{{ $navClass }}">
              ...
              <x-slot name="content">
                <div class="w-60">
                  ...
                  @can('laporanTransaksiAccess')
                  <x-jet-dropdown-link href="{{ route('laporanTransaksi.index') }}">
                    Laporan Transaksi
                  </x-jet-dropdown-link>
                  @endcan

                  @can('laporanTransaksiDivisiAccess')
                  <x-jet-dropdown-link href="{{ route('laporanTransaksiDivisi.index') }}">
                    Laporan Transaksi Per Divisi
                  </x-jet-dropdown-link>
                  @endcan
                </div>
              </x-slot>
